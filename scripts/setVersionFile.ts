...
function setHorizonWebVersion() {
  const content = {
    version: versions['horizon-web-vue'],
  };

  fs.writeFileSync(
    path.resolve(__dirname, '../packages/horizon-web-vue/src/styles/version.scss'),
    `$version: "${versions['horizon-web-vue'].split('.').slice(0, 2).join('-')}";`,
    'utf-8',
  );

  fs.writeFileSync(
    path.resolve(__dirname, '../packages/horizon-web-vue/src/version.json'),
