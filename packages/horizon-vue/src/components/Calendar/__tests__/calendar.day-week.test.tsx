...
describe('Calendar day/week public contracts', () => {
  ...
  test('day mode renders time formats, flags, disabled hours and scoped slots', async () => {
    ...
    const timezone = vi.fn((offset: number, date: string, rawDate: ReturnType<typeof dayjs>) =>
    ...
    expect(onPinFlagClick).toHaveBeenLastCalledWith(
      expect.objectContaining({ title: 'Clickable cross-day' }),
    );
  ...
  test('day and week timelines hide the hour label next to the current-time marker', () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date(2026, 1, 3, 12, 5));
    try {
      const day = mount(HCalendar, {
        props: {
          currentTimeLine: true,
          mode: 'day',
          modeSwitchableList: ['day'],
      ...
      expect(day.findAll('.h-calendar-day__body--time-label')[11].text()).toBe('');
      day.unmount();

      const week = mount(HCalendar, {
        props: {
          currentTimeLine: true,
          mode: 'week',
      ...
      expect(week.findAll('.h-calendar-week__body--time-label')[11].text()).toBe('');
      week.unmount();
    } finally {
      vi.useRealTimers();
    }
