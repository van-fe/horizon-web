...
import { groupCalendarRangesByLane, groupOverlappingCalendarRanges } from '@aurora/core';
import { ceilDay, floorDay, getMaxDate, getMinDate, isDatesRangeAreOverlap } from './timeHelper';
...
export default class PinFlagsHelper {
  ...
  public calculateFlagsInWeekdays() {
    ...
    for (let i = minDate; i.isBefore(maxDate); i = i.add(7, 'days')) {
      ...
      const flags = groupCalendarRangesByLane(
        this._pinFlags.value
          .filter(flag => isDatesRangeAreOverlap([flag._startAt!, flag._endAt!], [i, lastDate]))
          .map(flag => ({ flag, start: flag._startAt!.valueOf(), end: flag._endAt!.valueOf() })),
      ).map(lane => lane.map(item => item.flag));
  ...
  public calculateMergedFlags() {
    this.mergedFlags.value = groupOverlappingCalendarRanges(
      this._pinFlags.value.map(flag => ({
        flag,
        start: flag._startAt!.valueOf(),
        end: flag._endAt!.valueOf(),
      })),
    ).map(group => ({
      startAt: dayjs(Math.min(...group.map(item => item.start))),
      endAt: dayjs(Math.max(...group.map(item => item.end))),
      pinFlags: group.map(item => item.flag),
    }));
