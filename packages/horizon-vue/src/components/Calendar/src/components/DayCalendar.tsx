...
import { createCalendarTimelineScroller } from '@aurora/horizon-core';
...
export default defineComponent({
  ...
  setup() {
    ...
    const timelineScroller = createCalendarTimelineScroller({
      getScroller: () => ({ scrollTo: options => scrollbarRef.value?.scrollTo(options) }),
      getHourHeight: getOneHourHeightPx,
    });
    ...
    function scrollToHour(hour: number) {
      timelineScroller.scrollToHour(hour);
    }
    ...
    onBeforeUnmount(() => {
      timelineScroller.destroy();
      dragToCreateFlag.destroy();
    });
