...
const normalizeDrawerHtml = (html: string) => {
  ...
  return html.replace(
    /z-index: (\d+)/g,
    (_, rawValue: string) => `z-index: ${Number(rawValue) - offset}`,
  );
};
    ...
    expect(normalizeDrawerHtml(wrapper.html())).toMatchInlineSnapshot(`
      ...
      <div class="h-drawer" style="position: absolute; z-index: 2009;">
        ...
        <transition-stub name="h-drawer-slide-right" appear="true" persisted="true" css="true">
          <div role="dialog" aria-modal="true" aria-label="Drawer" tabindex="-1" class="h-drawer__container h-drawer--right" style="z-index: 2010; width: 418.667px; height: 100%; position: absolute;">
