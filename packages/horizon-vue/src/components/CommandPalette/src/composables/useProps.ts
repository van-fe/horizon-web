import {
  ...
  isCommandPaletteCommands,
  isCommandPaletteFilter,
} from '@aurora/core';
import type { CommandPaletteCommand, CommandPaletteFilter } from '@aurora/core';
import { declarePropType } from '@aurora/utils';
import type { ExtractPropTypes, PropType } from 'vue';
export interface CommandPaletteItem extends CommandPaletteCommand {
  keywords?: string[];
...
export const useCommandPaletteProps = declarePropType({
  ...
  /** 命令列表 @en Available commands. */ commands: {
    ...
    default: () => [...COMMAND_PALETTE_DEFAULTS.commands],
    validator: isCommandPaletteCommands,
  },
  ...
  /** 是否启用全局快捷键 Command/Ctrl+K @en Whether Command/Ctrl+K opens the palette. */ hotkey: {
    type: Boolean,
    default: COMMAND_PALETTE_DEFAULTS.hotkey,
  },
  /** 执行后是否关闭 @en Whether to close after command execution. */ closeOnSelect: {
    type: Boolean,
    default: COMMAND_PALETTE_DEFAULTS.closeOnSelect,
  },
  /** 自定义过滤函数 @en Custom command filter. */ filter: {
    type: Function as PropType<CommandPaletteFilter<CommandPaletteItem>>,
    validator: isCommandPaletteFilter,
  },
});
