...
export interface IndicatorOptions extends TabsProps {
...
export default function useResponsive(options: ToRefs<IndicatorOptions>) {
  const { wrapper, container, activeKey, size, type, root } = options as ToRefs<
  ...
  const updateActivateElement = async (mounted = false) => {
    ...
    if (elOffsetLeft <= clientWidth / 2) {
      ...
      const leftDelta = elOffsetLeft - clientWidth / 2 + elWidth / 2;
  ...
  useResize({ root, container, type }, () => {
    onRectChanged();
  });

  watch([activeKey, type, size, options.arrow, () => items.value.size], onRectChanged, {
    flush: 'post',
  });
