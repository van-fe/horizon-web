...
describe('Calendar helpers', () => {
  ...
  test('sorts days/ranges in both directions and finds extrema', () => {
    ...
    expect(sortDayRanges([c, c], [a, c], [a, b]).map(range => range[1].date())).toEqual([
      2, 3, 3,
    ]);
  ...
  test('floors/ceils integrated dates and builds CSS time geometry', () => {
    ...
    expect(getIntegratedDay(noon).format('YYYY-MM-DD HH:mm:ss')).toBe(
      '2026-01-01 00:00:00',
    );
    expect(getIntegratedDay(noon, false).format('YYYY-MM-DD HH:mm:ss')).toBe(
      '2026-01-02 00:00:00',
    );
    ...
    expect(getHoursDiffCssRaw(d('2026-01-01 01:00'), d('2026-01-01 03:00'))).toContain(
      '* 7200',
    );
  ...
  test('computes overlap/common/excluded range contracts for every relative position', () => {
    ...
    expect(getDatesRangeExcludeParts(current, r('2026-02-01', '2026-02-02'))).toEqual([
      current,
    ]);
  ...
  test('PinFlagsHelper groups overlaps by week/day and exposes query contracts', () => {
    ...
    expect(
      helper.getFlagsAmountInHour(d('2030-01-01'), d('2030-01-02'), d('2030-01-01')),
    ).toBe(0);
  });
  ...
  test('PinFlagsHelper temp lifecycle emits, updates borders and removes listeners', () => {
    ...
    const neighboring = helper.addTempPinFlag(
      d('2026-02-01 03:00'),
      d('2026-02-01 04:00'),
    );
  ...
  test('DragToCreateFlag guards disabled state and reserves true/object/false results', async () => {
    ...
    for (const reserve of [
      true,
      false,
      { title: 'Modified', type: 'success' as const },
    ]) {
      ...
      if (reserve === false) {
        ...
        expect(flags.pinFlags.value[0].title).toBe(
          reserve === true ? 'Creating' : 'Modified',
        );
      }
    }
  });
  ...
  test('DragToCreateFlag follows pointer geometry, cross-day and disabled-hour policies', async () => {
    ...
    for (const [through, clientY] of [
      ...
      const props = {
        ...
        disableHours: () => [
          [dayStart.add(2, 'hour'), dayStart.add(4, 'hour')],
        ],
      ...
      drag.onMousedown(
        { target: body, offsetY: 3600 } as unknown as MouseEvent,
        dayStart,
      );
    ...
    reverseDrag.onMousedown(
      { target: body, offsetY: 18_000 } as unknown as MouseEvent,
      dayStart,
    );
    document.documentElement.dispatchEvent(new MouseEvent('mousemove', { clientY: 3600 }));
    document.documentElement.dispatchEvent(new MouseEvent('mouseup'));
