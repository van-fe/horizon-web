...
describe('Notification styles', () => {
  test('keeps the box shadow connected to an existing design token', () => {
    const globalVariablesCss = compile(
      resolve(stylesRoot, 'global-variables.scss'),
      testScssOptions,
    ).css;
    const notificationCss = compile(
      resolve(notificationRoot, 'src/style/index.scss'),
      testScssOptions,
    ).css;

    expect(globalVariablesCss).toContain('--h-shadow-down:');
    expect(globalVariablesCss).toContain('--h-notification-box-shadow: var(--h-shadow-down);');
