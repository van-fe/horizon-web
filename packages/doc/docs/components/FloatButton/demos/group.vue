  ...
  <div>
    <h-form label-position="left" label-vertical-align="middle" label-width="150px" helper-placement="after-label" helper-theme="dark" style="width: 400px">
      ...
      <h-form-item label="是否开启折叠">
        <h-switch v-model="useCollapse" :status="true" status-oh-text="开启" status-off-text="关闭" />
      </h-form-item>
      <h-form-item label="是否可拖拽">
        <h-switch v-model="draggable" :status="true" status-oh-text="开启" status-off-text="关闭" />
      </h-form-item>
      <h-form-item label="是否允许吸附在底部" helper="如果拖拽距离离底部较近的话，则将其吸附在底部">
        <h-switch v-model="adsorbBottom" :status="true" status-oh-text="允许" status-off-text="禁止" />
      </h-form-item>
