### 基本用法
与其他选择器组件一致，都会有 `normal` `no-border` 样式
...
:::demo ./demos/check-all-summary.vue :::
...
:::demo ./demos/expand-strictly.vue :::
...
:::demo ./demos/display-way.vue :::

### 面板展开方式
设置 `trigger = 'hover'` 即可在悬浮时打开面板
:::demo ./demos/trigger-hover.vue :::
...
:::demo ./demos/filterable.vue :::

### 过滤全选
支持传入 `use-filter-check-all = true`，在过滤时开启全选

:::demo ./demos/filter-check-all.vue :::
...
:::demo ./demos/filterable-config.vue :::

### 过滤后自定义展示
通过 `searchPanelRender` 插槽，可以自定义过滤的内容
:::demo ./demos/filter-render-slot.vue :::
...
:::demo ./demos/custom-trigger-inner.vue :::

### 自定义选中 tag
常用在多选中，使用 `tagRender` 插槽自定义被选中的 `tag`
:::demo ./demos/custom-selected-item.vue :::
...
:::demo ./demos/custom-option-render.vue :::

### 自定义选择器
借助 `default` 插槽，可以自定义选择器
:::demo ./demos/custom-trigger.vue :::

### model-value 未匹配
当 `model-value` 无法在 options 中找到时，会直接展示其 `value` 值
:::demo ./demos/unmatched-value.vue :::
...
### 触底加载
如果某子项数据非常大，一次性从服务器加载比较耗费时间，则可以借助 `panelReachBottom` 事件做到类似于分页加载的效果
:::tip 注意：必须有滚动才可以触发 `触底事件`，如果没有滚动则不会抛出 `panelReachBottom` 事件 :::
