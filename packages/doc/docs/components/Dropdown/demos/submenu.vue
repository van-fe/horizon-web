<template>
  <h-row>
    <h-col :span="24" @click="onClick">
      <h-dropdown :size="size" @visible-change="v => (visible1 = v)" @command="onCommand">
        <h-button>
          Grade (hover)
          <template #suffix>
        ...
        <template #dropdown>
          <h-dropdowh-menu>
            <h-dropdowh-submenu v-for="item of list" :key="item.label" :title="item.label">
              <h-dropdowh-submenu
...
const size = ref<DropdownProps['size']>('medium');
