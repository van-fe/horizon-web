...
describe('unplugin-resolver', () => {
  ...
  test('component', () => {
    for (const comp of components) {
      ...
      expect(res?.from).toMatch(/^@aurora\/horizon-vue\//);
  ...
  test('uses the Vue renderer package for Vite style imports', () => {
    ...
    expect(resolver.libraryName).toBe('@aurora/horizon-vue');
    expect(resolver.resolveStyle('h-button')).toBe(
      '@aurora/horizon-vue/es/components/Button/src/style/index.css',
    );
  });

  test('resolves explicit renderer packages without cross-renderer paths', () => {
    expect(getHorizonWebRendererPackage('vue')).toBe('@aurora/horizon-vue');
    expect(getHorizonWebRendererPackage('react')).toBe('@aurora/horizon-react');
    expect(resolveHorizonWebPackageImport('vue', 'Button')).toMatchObject({
      name: 'HButton',
      from: '@aurora/horizon-vue/es/components/Button',
    });
    expect(resolveHorizonWebPackageImport('react', 'Button')).toEqual({
      renderer: 'react',
      packageName: '@aurora/horizon-react',
      name: 'Button',
      from: '@aurora/horizon-react',
      sideEffects: ['@aurora/horizon-react/style.css'],
    });
  });
  ...
  test('provides a React style resolver with exclusion and opt-out', () => {
    ...
    expect(resolver.libraryName).toBe('@aurora/horizon-react');
    expect(resolver.resolveStyle('select')).toBe('@aurora/horizon-react/style.css');
