              ...
              propTypeDefined?.forEachChild(node => {
                switch (node.getKind()) {
                  ...
                  case ts.SyntaxKind.ArrowFunction:
                    res.type = node.getText();
                    ({ params: res.params } = analyseFunctionExpression(node, jsDoc, fileElements));
                    break;
                }
              });
              break;
...
export default function analyseOptions(
  ...
  if (directiveInfo.optionsVariableFilePath) {
    const filePathWithExtName = completeFileExtName(directiveInfo.optionsVariableFilePath);

    if (!filePathWithExtName) {
      throw new Error(`This file cannot be found: ${directiveInfo.optionsVariableFilePath}.ts(x)`);
    }
    ...
    if (sourceFile) {
      ...
      if (fileElements.variables[directiveInfo.optionsVariableName]) {
        const variable = fileElements.variables[directiveInfo.optionsVariableName];
        ...
        if (variable) {
          ...
          if (variable.getKind() === ts.SyntaxKind.ObjectLiteralExpression) {
            ...
            // 2. defineXXX(() => true)
            const rawVariable =
              fileElements.variablesWithRawDeclaration[directiveInfo.optionsVariableName];
