...
import { legoEsmOutput, legoLibOutput } from '@nio-fe/shared/plugins';
...
export default definePlugin(opts => ({
  ...
  async writeBundle(options, bundle) {
    ...
    const getDependencies = (importName: string): string[] => {
      if (importName in bundle) {
        const imports = filterImports((bundle[importName] as any).imports);

        return [importName, ...imports.map(item => getDependencies(item)).flat()];
      }
