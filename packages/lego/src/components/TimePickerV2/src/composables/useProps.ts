...
export const useTimePickerV2Props = declarePropType({
  /**
   * 双向绑定值
   */
  modelValue: {
   ...
   */
  disabledTime: {
    type: Function as PropType<NTimePickerV2DisabledTimeMethodType>,
  },
  /**
   * 弹出层样式
   */
...
export const useTimePickerV2TimePanelProps = declarePropType({
  /**
   * 输入值
   */
  modelValue: {
   ...
   */
  dateType: {
    type: String as PropType<TimePickerV2Props['type']>,
    required: true,
  },
  /**
   * 时间步长，单位为分钟，在 `type = 'time'` 时有效
...
export type TimePickerV2Props = ExtractPropTypes<typeof useTimePickerV2Props>;
export type TimePickerV2TimePanelProps = ExtractPropTypes<typeof useTimePickerV2TimePanelProps>;
