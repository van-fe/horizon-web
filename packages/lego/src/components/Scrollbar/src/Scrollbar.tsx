...
import { ComponentClassBlock, cls, sizeUnitTransform, useNamespace } from '@nio-fe/shared';
...
import { NHover } from '~/components/Hover';
...
export default defineComponent({
  ...
  setup(
    ...
    function updateSize() {
      ...
      wrapSize.value.width = rect?.width ? Math.ceil(rect?.width) : 0;
      wrapSize.value.height = rect?.height ? Math.ceil(rect?.height) : 0;

      viewSize.value.width = wrapRef.value?.scrollWidth || 0;
      viewSize.value.height = wrapRef.value?.scrollHeight || 0;
    }
      ...
      >
        {{
          default: ({ hover }: { hover: boolean }) => (
            <div
              ref={scrollbarRef}
              class={cls(classHelper.block, classHelper.m(size.value))}
              {...attrs}
            >
              <div
                ref={wrapRef}
