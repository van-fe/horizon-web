import type { Awaitable, Data } from '@nio-fe/shared';
import type { NUploadV2FileType } from '../utils/fileDefines';

export interface NUploadV2Chunk {
  /**
   * 分片下表
   */
  index: number;
...
export interface NUploadV2MultipartSetting {
  /**
   * 同时最多上传数量
   * @default 5
   */
   ...
   * @param file 待上传文件
   */
  initUpload?: (file: NUploadV2FileType) => Awaitable<Data>;
  /**
   * 分片文件名处理
   ...
   * @param part 当前正在上传的文件片段
   */
  beforePartUpload?: (file: NUploadV2FileType, index: number, part: Blob) => Data;
  /**
   * 处理合并文件，在所有part都上传完后会回调
   * @param file 上传的文件
   * @param chunks 所有已上传的分片的文件
   */
  handleMerge: (file: NUploadV2FileType, chunks: NUploadV2Chunk[]) => Promise<void>;
}
