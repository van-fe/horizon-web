...
import { NFormDisabledInjectedKey } from '~/components/Form';

export default defineComponent({
  name: 'UploadDropArea',
  setup() {
    const classHelper = new ComponentClassBlock('upload-v2--drop-area');
          ...
          <div class={cls(classHelper.e('inner'))}>
            <div class={classHelper.e('title')}>
              <div class={classHelper.em('title', 'icon')}>
                {slots.icon?.() ?? (
                  <IconAdd size={cssVariable('upload-v2', 'size', 'drop-area', 'icon')} />
                )}
              </div>
