import { arrayableToArray, getRemoteUrlFileHeader, isObject } from '@nio-fe/shared';
import type { Arrayable } from '@nio-fe/shared';
import type { NUploadV2FileType, NUploadV2RawFileType, NUploadV2UserFile } from './fileDefines';
import { NUploadV2FileStatusEnum, fileTypeMapping, NUploadV2FileTypeEnum } from './fileDefines';
import { nanoid } from 'nanoid';
import { warn } from '~/utils/useLog';

export function isUploadUserFile(data: unknown): data is NUploadV2UserFile {
  return isObject(data) && 'name' in data && 'url' in data;
}

export function isUploadV2File(data: unknown): data is NUploadV2FileType {
...
export function getFileTypeByExt(ext: string): NUploadV2FileTypeEnum {
  ext = ext.replace(/^\./, '').toLowerCase();

  const type = Object.keys(fileTypeMapping).find(curr =>
    fileTypeMapping[curr as NUploadV2FileTypeEnum].includes(ext),
  ) as NUploadV2FileTypeEnum | undefined;

  return type ?? NUploadV2FileTypeEnum.Unknown;
}
...
export function isFileSame(rawFile: NUploadV2RawFileType, file: NUploadV2FileType) {
  if (isUploadV2File(rawFile)) {
...
export function uploadFileTransformToBasicType(file: NUploadV2RawFileType): NUploadV2UserFile {
  if (isUploadV2File(file)) {
    return file;
  }
...
export function updateFileList(
  newFiles: Arrayable<NUploadV2RawFileType>,
  oldFiles: Arrayable<NUploadV2RawFileType> | undefined | null,
) {
...
export async function getAndSetFileSize(file: NUploadV2UserFile) {
  try {
    file.size = await getRemoteUrlFileHeader(file.url);
  } catch (e) {
    warn('upload-v2', `Cannot get file size from url: ${file.url}`);
  }
}

export function transformSingleRawFileTypeToUploadV2FileType(target: NUploadV2RawFileType) {
  if (isUploadV2File(target)) {
    return target;
    ...
    return {
      ...
      uuid: nanoid(),
      size,
      status: target.url ? NUploadV2FileStatusEnum.Success : NUploadV2FileStatusEnum.New,
    };
  } else {
    return {
      name: target.name,
      ...
      uuid: nanoid(),
      size: target.size,
      status: NUploadV2FileStatusEnum.New,
      percentage: 0,
      url: '',
