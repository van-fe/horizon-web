...
import NTransferV2 from '../';
import NTransferV2Panel from '../src/TransferV2Panel';
...
describe('TransferV2.tsx', () => {
  ...
  test('basic', async () => {
    const wrapper = shallowMount(() => <NTransferV2 data={getTestData()} />);
    expect(wrapper.findComponent(NTransferV2)).toBeTruthy();
  });

  test('default target list', () => {
    const value = ref([1, 4]);
    const wrapper = mount(() => <NTransferV2 v-model={value.value} data={getTestData()} />);
    const panelELE = wrapper.findComponent(NTransferV2Panel);
  ...
  test('filterable', async () => {
    ...
    const wrapper = mount(() => (
      <NTransferV2 v-model={value.value} filterable data={getTestData()} />
    ));
    const panelELE = wrapper.findComponent(NTransferV2Panel);
    const inputELE = panelELE.find('.n-input__inner');
    await inputELE.setValue('10');
    const checkboxELEArr = wrapper.findAll('.n-transfer-v2-panel__item > .n-checkbox');
  ...
  test('transfer', async () => {
    const value = ref([1, 4]);
    const wrapper = mount(() => <NTransferV2 v-model={value.value} data={getTestData()} />);
    expect((wrapper.findComponent('.n-transfer-v2--right') as VueWrapper<any>).vm.data.length).toBe(
      value.value.length,
    );
    value.value = [1, 2, 4];
    await nextTick();
    ...
    const clearButton = wrapper.find('.n-transfer-v2__header--clear');
  ...
  test('customize', () => {
    ...
    const label = wrapper.find('.n-transfer-v2__header .n-checkbox__label');
  ...
  test('placeholder', () => {
    ...
    const transferELE = wrapper.findComponent(NTransferV2);
  ...
  test('internal', async () => {
    ...
    const wrapper = mount(
      () => <NTransferV2 v-model={state.value} filterable={true} data={getTestData()} />,
      {
        global: {
          provide: {
  ...
  test('disabled', async () => {
    ...
    const wrapper = mount(() => (
      <NTransferV2 data={getTestData()} v-model={value.value} disabled={disabled.value} />
    ));
    ...
    const clearButtonELE = wrapper.find('.n-transfer-v2__header--clear');
    ...
    const clearButtonELENew = wrapper.find('.n-transfer-v2__header--clear');
