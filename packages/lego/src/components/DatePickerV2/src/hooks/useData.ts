...
import type { NDatePickerDomRefs, NDatePickerV2BaseSupportType } from '../utils/types';
  ...
  function refreshShowValue() {
    ...
    if (propRefs.formatTriggerText?.value && isFunction(propRefs.formatTriggerText.value)) {
      ...
      if (
        ...
        warn('date-picker-v2', `formatTriggerText returned value is not array!`);
