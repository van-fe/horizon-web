...
import { cls, ComponentClassBlock, cssVariable, isObject, useNamespace } from '@nio-fe/shared';
...
import type { NCalendarPinFlag } from '~/components/Calendar';
import {
                          ...
                          {parentSlots.dateCellTitle?.(date.format(props.format), date) ?? (
                            <div class={classHelper.em('day', 'header')}>
                              <div class={classHelper.em('day', 'header-number')}>
                                {date.format('D') === '1'
                                  ? locale.value?.langService.td().lego.calendar.simpleMonths[
                                      Number(date.format('M')) - 1
                                    ] +
                                    date.format('D') +
                                    locale.value?.langService.td().lego.calendar.simpleDay
                                  : date.format('D')}
                              </div>
                            </div>
                          )}
