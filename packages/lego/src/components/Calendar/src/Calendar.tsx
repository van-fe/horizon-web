...
import { NTab, NTabs } from '~/components/Tabs';
import { NButton } from '~/components/Button';
import PinFlagsHelper from './utils/PinFlagsHelper';
import { NDatePickerV2 } from '~/components/DatePickerV2';
    ...
    const switchModes = computed(() => {
      ...
      list.forEach(item => {
        switch (item) {
          case 'year':
            nodes.push(
              <NTab label={locale.value?.langService.td().lego.calendar.year} key="year" />,
            );
            break;
          case 'month':
            nodes.push(
              <NTab label={locale.value?.langService.td().lego.calendar.month} key="month" />,
            );
            break;
          case 'week':
            nodes.push(
              <NTab label={locale.value?.langService.td().lego.calendar.week} key="week" />,
            );
            break;
          case 'day':
            nodes.push(<NTab label={locale.value?.langService.td().lego.calendar.day} key="day" />);
            break;
        }
      });
        ...
        <div class={classHelper.e('header')}>
          {slots.switchDate?.(currentFormatDate.value, currentDate.value) ?? (
            <div class={classHelper.em('header', 'switcher-wrapper')}>
              ...
              >
                {locale?.value?.langService.td().lego.datePicker.today}
              </NButton>
