  ...
  setup(props, { emit }) {
    ...
    const list = computed(() => {
      ...
      if (pickerOptions.value.start && pickerOptions.value.step && pickerOptions.value.end) {
        ...
        while (NDatejs.compareTime(current, pickerOptions.value.end) <= 0) {
          ...
          const isSelected = !isDisabled && handleIsSelected(currentPanelTime);
    ...
    function handleIsSelected(currentPanelTime: PanelTimeType) {
      if (isMultipleSingleTime?.value) {
        return (
          (panelTime.value as PanelTimeType[])?.findIndex(
            item =>
              item.hours === currentPanelTime.hours && item.minutes === currentPanelTime.minutes,
          ) > -1
        );
      }
      return (
        (panelTime.value as PanelTimeType).hours === currentPanelTime.hours &&
