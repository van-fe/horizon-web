...
import type { Awaitable } from '@nio-fe/shared';
import { declarePropType, isNumber } from '@nio-fe/shared';
...
export const useTagGroupProps = declarePropType({
  ...
  minDisplayed: {
    ...
    validator: (val: unknown) => {
      if (!isNumber(val)) {
        console.error('[lego tag-group]: You must pass number value to minDisplayed.');
        return false;
      }

      if (val < 0) {
        console.error(`[lego tag-group]: You can't pass number less than 0 to minDisplayed.`);
        return false;
      }
