...
import type { ModelValueSingleType, OptionProps, SelectProps } from '../composables/useProps';

export default function useCheckAll(
  propRefs: ToRefs<SelectProps>,
  options: {
    optionList: Map<OptionProps['value'], SelectCollectedOptionData<'option'>>;
  ...
  const checkAllCountShowText = computed(() => {
    if (propRefs.useCheckAllCount.value) {
      if (propRefs.checkAllCountConsiderFilter.value) {
        ...
        return options.presetModelValueSet.value.size > 0
          ? `${options.presetModelValueSet.value.size}/${options.optionList.size}`
          : options.optionList.size.toString();
      }
    } else {
      return '';
    }
