...
#### M6-B18 第十八批实施记录（2026-08-12）

| 任务 | 状态 | 交付 |
| --- | --- | --- |
| CORE-B18-001 | Done | Backtop 的公共 props、默认值、非负距离校验器、点击事件、内容区域、命令及 manifest 迁入 `@aurora/core`；阈值判断、三次缓动和帧偏移算法只定义一次 |
| WEB-B18-001 | Done | 在同名 Web Core 目录提供 Window/HTMLElement 目标解析、滚动偏移读写、显示状态同步、防抖监听与可取消滚动控制器；无效选择器回退窗口，减少动态效果偏好直接到顶，销毁时清理计时器和动画帧 |
| VUE-B18-001 | Done | Vue props/emits/slots/exposes 通过类型化适配消费公共契约，保留选择器 target、Transition、AIcon 与历史 click 事件；移除本地滚动算法和 VueUse 监听副本，改为语义 button，并新增 `scrollToTop()` / `focus()` 命令 |
| REACT-B18-001 | Done | 完成原生 button Backtop、字符串/元素/窗口/解析函数目标、阈值显隐、共享平滑滚动、自定义内容、Provider 可访问文案、原生属性及命令 ref；SSR 初始渲染不访问 DOM |
| DS-B18-001 | Done | Backtop Sass 与变量迁入 `@aurora/theme`，Vue 使用包导出代理，React 使用同源样式；统一原生 button reset、focus-visible、disabled 状态和暗色语义 token |
| DOC-B18-001 | Done | Backtop 中英文 React 页面、2 个独立 TSX 示例、Vue API/命令说明与双语侧边栏入口完成；45 组 Vue/React 契约生成且组件文档保持 renderer 原生表达 |
| QA-B18-001 | Done | Core 152、Web Core Node 10/Chromium 43、React Chromium 166、Vue Chromium 2319 项（2318 通过、1 项预期失败）全部通过，覆盖率为 97.93% / 95.04% / 97.71% / 98.25%；Vue Node 23、resolver 12、colors 1、upload adapters 66 项通过；五包构建、文档与 Vue/React/SSR/tree-shaking 消费工程通过 |

#### M6-B19 第十九批实施记录（2026-08-12）

| 任务 | 状态 | 交付 |
| --- | --- | --- |
| CORE-B19-001 | Done | Affix 的公共 props、默认值、位置枚举、数值校验器、状态事件、内容区域、命令及 manifest 迁入 `@aurora/core`；顶部/底部边界判断和固定几何计算只定义一次 |
| WEB-B19-001 | Done | 在同名 Web Core 目录提供选择器/元素/窗口目标解析，以及测量、占位、滚动监听、ResizeObserver 响应式重排和完整销毁控制器；无效选择器安全回退窗口，自定义滚动容器使用其可视边界 |
| VUE-B19-001 | Done | Vue props/emits/slots/exposes 通过类型化适配消费公共契约，保留 `target` 历史接口和 `updatePosition()` 命令；移除本地 DOM 测量、监听与 VueUse ResizeObserver 副本，新增固定状态 `change` 事件 |
| REACT-B19-001 | Done | 完成 React Affix，支持字符串/元素/窗口/解析函数目标、顶部与底部固定、响应式占位、原生 div 属性、状态回调和命令 ref；初始 SSR 渲染不访问 DOM |
...
#### M6-B25 第二十五批实施记录（2026-08-12）

| 任务 | 状态 | 交付 |
| --- | --- | --- |
| CORE-B25-001 | Done | Form/FormItem 的公共 props、默认值、校验器、事件、区域、命令、嵌套字段路径、required 标记与触发时机算法迁入 `@aurora/core`；表单与字段 controller 统一字段注册、校验聚合、初始值恢复和错误清理，并由 Core 直接封装 `async-validator` |
| WEB-B25-001 | Done | 在同名 Web Core 目录提供字段滚动定位能力，统一 `scrollIntoView` 行为并以真实 Chromium 覆盖，不在 renderer 内重复浏览器调用 |
| VUE-B25-001 | Done | Vue Form/FormItem 的 props/emits/slots/exposes 类型化消费公共契约与 controller，保留历史 `model`、`prop`、校验返回值、Grid、helper、required 文案和滚动行为；移除 renderer 对 `async-validator` 的直接依赖 |
| REACT-B25-001 | Done | 完成 React Form/FormItem 与 compound `Form.Item`，支持嵌套字段、同步/异步规则、表单与字段命令 ref、受控模型、Grid、render state、原生提交以及 Input/Checkbox/Radio/Rate/Select/Slider/Switch 的禁用、错误、ARIA 与 change/blur 校验接入 |
