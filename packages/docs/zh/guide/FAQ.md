...
## 1. 提示 `request to https://registry.npmjs.org/@nio-fe%21flego failed` {#registry}

是因为没有设置源 (`registry`)，目前 `@nio-fe/lego` 相关包只发布在了公司内网，请选用以下任一方式设置：
...
- 或者安装时特别指定：
    ```shell
    $ npm i @nio-fe/lego --registry https://npmmirror.nioint.com/
    ```
  
## 2. 构建好项目后，使用指令时出现 `TypeError: Cannot read properties of undefined (reading 'deep')` 的报错： {#version-same}

`@nio-fe/lego` 和 `@nio-fe/unplugin-resolver` 的版本号一定要保持一致
...
使用 `vue-cli` 构建项目，一般情况下会出现不支持 `optional-chaning` 等ES7语法的情况，所以需要按照如下配置：
  ...
  ```
  2. 在 `vue.config.js` 中，增加以下语句，使 `babel-loader` 也转译 `@nio-fe/xx` 的包
  ```ts
  module.exports = {
    // ...
