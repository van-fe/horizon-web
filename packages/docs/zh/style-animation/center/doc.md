...
## 文本居中
使用 [Typography](./typography) 中的 `text-center` 工具类实现水平居中，通过设置 `line-height` 实现垂直居中。
|Class|Properties|
|-|-|
...
## 自动边距居中
如果元素具有固定的 `width`，可以使用 [Spacing](./spacing) 中的 `mx-auto` 工具类实现水平居中。
|Class|Properties|
|-|-|
...
## 定位居中
如果元素是 `absolute` 或 `fixed` 定位，可以使用 [Position](./position) 中的工具类实现水平或垂直居中。
|Class|Properties|
|-|-|
...
## Flex 居中
如果元素是 `flex` 或 `inline-flex` 模型，可以使用 [Flex](./flex) 中的工具类实现水平或垂直居中。
|Class|Properties|
|-|-|
