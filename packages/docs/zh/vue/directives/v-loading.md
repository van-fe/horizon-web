...
:::demo vue/directives/v-loading/demo1.vue :::

## 局部与全屏加载

加载指令可以覆盖任意内容容器，也可以通过 `fullscreen` 展示页面级等待状态。全屏示例会在 2 秒后自动关闭。

:::demo vue/directives/v-loading/demo2.vue :::

## 动态配置

组合配置项可以即时调整加载动画，适合根据不同业务场景统一加载反馈。

:::demo vue/directives/v-loading/demo3.vue :::

## 延迟显示

当请求很快完成时，立即展示 loading 可能产生闪屏。通过 `delay` 设置延迟时间，可以让短于该时间的请求不显示加载遮罩。

:::demo vue/directives/v-loading/delay.vue :::
...
```javascript
import { loading } from '@aurora/horizon-vue';
...
```ts
import { LoadingService } from '@aurora/horizon-vue';
