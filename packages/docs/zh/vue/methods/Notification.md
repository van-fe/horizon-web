...
:::demo vue/methods/Notification/basic.vue :::
...
:::demo vue/methods/Notification/content.vue :::
...
:::demo vue/methods/Notification/different-types.vue :::
...
:::demo vue/methods/Notification/placement.vue :::

## Offset

使用 `offset` 设置偏移量，相对屏幕顶部或底部的偏移量，使用 `gap` 设置相邻Notification实例之间的间距。

:::demo vue/methods/Notification/offset.vue :::

## Width

使用 `width` 设置自定义宽度。

:::demo vue/methods/Notification/width.vue :::
...
:::demo vue/methods/Notification/operation.vue :::

## Show close

通过`show-close`属性可以设置关闭按钮是否显示。

:::demo vue/methods/Notification/close.vue :::

## Close/CloseAll

异步调用 `$notify` 会返回当前 Notification 的实例。如果需要手动关闭实例，可以调用它的 `close` 方法，也可调用 `closeAll` 方法关闭所有实例。

:::demo vue/methods/Notification/methods.vue :::
