...
`,
  ...
  _push(`<h2 id="关闭和增加" tabindex="-1">关闭和增加 <a class="header-anchor" href="#关闭和增加" aria-label="Permalink to &quot;关闭和增加&quot;">​</a></h2><p>通过设置 <code>n-tabs (editable)</code>、<code>n-tab (closable)</code> 属性可以开启动态增减选项卡。仅在 <code>line | card | page</code> 生效；关闭和新增的逻辑由业务实现。 <code>v2 = true</code>，删除如果是当前选项卡，则默认选中第一个选项卡</p>`);
  _push(ssrRenderComponent(_component_demo_block, {
    source: `<script setup lang="ts">
import { $message, type HTabValue } from '@aurora/horizon-web';
...
`,
  ...
  _push(`<h2 id="结合-n-panel-使用" tabindex="-1">结合 <code>n-panel</code> 使用 <a class="header-anchor" href="#结合-n-panel-使用" aria-label="Permalink to &quot;结合 \`n-panel\` 使用&quot;">​</a></h2><p>使用面板组件，开发选项卡应用</p>`);
  _push(ssrRenderComponent(_component_demo_block, {
    source: `<template>
...
`,
  ...
  _push(`<h2>Tabs Api</h2><h3>Tabs Props</h3><table class="md-table"><thead><tr><th>属性</th><th>说明</th><th>类型</th><th style="${ssrRenderStyle({ "min-width": "50px" })}" class="text-center">必填</th><th>默认值</th></tr></thead><tbody><tr><td style="${ssrRenderStyle({ "word-break": "keep-all" })}">v-model`);
