...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  _push(`<div${ssrRenderAttrs(_attrs)}><h1>Form</h1><p class="description">由输入框、选择器、单选框、多选框等控件组成，用以收集、校验、提交数据</p><h2 id="基础表单" tabindex="-1">基础表单 <a class="header-anchor" href="#基础表单" aria-label="Permalink to &quot;基础表单&quot;">​</a></h2><p>在 <code>h-form-item</code> 的 <code>label</code> 属性上设置表单项的标签，默认情况下，标签展示在上方。</p>`);
...
`,
  ...
  _push(`<h2 id="大小" tabindex="-1">大小 <a class="header-anchor" href="#大小" aria-label="Permalink to &quot;大小&quot;">​</a></h2><p>可以通过 <code>h-form</code> 控制组件大小</p><p><code>size</code> 可以覆盖通过 <code>h-application</code> 设置的 <code>size</code></p>`);
  _push(ssrRenderComponent(_component_demo_block, {
    source: `<template>
  <h-row>
    <h-col>
...
`,
  ...
  _push(`<h2 id="行内表单" tabindex="-1">行内表单 <a class="header-anchor" href="#行内表单" aria-label="Permalink to &quot;行内表单&quot;">​</a></h2><p>如果表单项数量较少，且都是 <code>h-input</code> 这样高度较小的简单组件，可以通过设置 <code>inline</code> 为 <code>true</code> 启用行内表单。</p>`);
  _push(ssrRenderComponent(_component_demo_block, {
    source: `<template>
  <h-form inline>
    <h-form-item label="User name">
...
`,
  ...
  _push(`<h2 id="添加验证规则" tabindex="-1">添加验证规则 <a class="header-anchor" href="#添加验证规则" aria-label="Permalink to &quot;添加验证规则&quot;">​</a></h2><p>你可以给表单添加验证规则，以判断表单项的绑定值是否符合预期。<br> 首先给 <code>h-form</code> 组件设置 <code>model</code> 属性，这是整个表单域中所有绑定字段的集合。 然后给需要校验的的 <code>h-form-item</code> 添加 <code>prop</code> 属性，它应该是 <code>model</code> 中的字段名，并给 <code>rules</code> 属性传入验证规则。详细用法参见 <a href="https://github.com/yiminghe/async-validator#rules" target="_blank" rel="noreferrer">async-validator#rules</a>。</p>`);
  _push(ssrRenderComponent(_component_demo_block, {
    source: `<template>
...
`,
  ...
  _push(`<h2 id="验证后清空验证结果" tabindex="-1">验证后清空验证结果 <a class="header-anchor" href="#验证后清空验证结果" aria-label="Permalink to &quot;验证后清空验证结果&quot;">​</a></h2><p>验证某个表单后，如果想清空验证结果，可以对 <code>h-form</code> 使用 <code>clearValidate</code> 去清空验证结果，也可以针对 <code>h-form-item</code> 使用<code>clearValidate</code> 去清空验证结果</p>`);
  _push(ssrRenderComponent(_component_demo_block, {
    source: `<template>
  <h-row>
    <h-col>
...
`,
  ...
  _push(`<h2 id="只做显示组件" tabindex="-1">只做显示组件 <a class="header-anchor" href="#只做显示组件" aria-label="Permalink to &quot;只做显示组件&quot;">​</a></h2><p>在结合一些表单组件使用时（例如 <code>formily</code>），有自己的验证规则，此时就不需要 <code>h-form</code> 做验证，所以可以配置 <code>only-render</code> 来设定是否只作为渲染组件</p><p>当设置 <code>form.only-render</code> 为 <code>true</code> 且 <code>form-item.error</code> 有变化时，会立刻根据 <code>form-item.error</code> 是否为空而标注表单元素为错误状态</p><p>需要注意的是，此时 <code>emit.validateChange</code> 不会触发</p>`);
  _push(ssrRenderComponent(_component_demo_block, {
    source: `<template>
...
`,
  ...
  _push(`<h2 id="使用自定义表单组件" tabindex="-1">使用自定义表单组件 <a class="header-anchor" href="#使用自定义表单组件" aria-label="Permalink to &quot;使用自定义表单组件&quot;">​</a></h2><p>如果使用了 <code>horizon-web</code> 外的表单组件，但也需要用到 <code>h-form</code> <code>h-form-item</code> 的验证功能，直接使用提供的 <code>provide</code> 值来做即可</p><p>只需要 <code>inject(&#39;HFormItemTriggerInjectedKey&#39;)</code>，在表单有 <code>change</code> 或 <code>blur</code> 事件时调用即可</p>`);
  _push(ssrRenderComponent(_component_demo_block, {
    source: `<template>
  <h-form ref="formRef" :model="formData" validate-trigger="blur">
    <h-form-item
