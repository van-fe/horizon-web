...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    const createData = (amount, childAmount, level = 0) => new Array(amount).fill(0).map(() => ({
      ...
      isLeaf: childAmount <= 0 ? level >= 4 ? true : faker.datatype.boolean() : void 0
    }));
    const data = ref(createData(20, 6));
    function dynamicLoad(rowData) {
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("demos/components/Table/tree.vue");
