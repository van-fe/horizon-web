...
`,
  ...
  _push(`<h2 id="折叠方式" tabindex="-1">折叠方式 <a class="header-anchor" href="#折叠方式" aria-label="Permalink to &quot;折叠方式&quot;">​</a></h2><p>配置 <code>display-type</code> 来控制折叠方式</p><p><code>full</code>: 全部展示，如果超出父级宽度，则会换行展示</p><p><code>ellipsis</code>: 省略展示，超出父级宽度，则将从第二个开始的元素开始收起，直到不超出父级宽度</p><p><strong>要注意的是，使用 <code>ellipsis</code> 显示模式，且使用 <code>n-breadcrumb-item</code> 渲染时，一定要给每一个 <code>n-breadcrumb-item</code> 设置一个唯一的 <code>key</code>，否则渲染内容可能会出现错误</strong></p>`);
  _push(ssrRenderComponent(_component_demo_block, {
    source: `<template>
